...
Page({
  ...
  onLoad: function (e) {
    ...
    if(!e.scene) {
      ...
      this.setData({currentTab: e.currentTab})
      request({url:'/WeekActivity/WeekCategory',data:{categoryId:e.WeekCategoryId}})
      .then(res=> {
  ...
  getNavData (name) {
    ...
    .then(res=> {
      ...
      let childid = navData[0].WeekCategoryId
      this.setData({
        navData,
        productList,
        datalist,
  ...
  switchNav(event) {
   ...
   this.setData({
     ...
     childid: event.currentTarget.dataset.weekcategoryid
    })
  ...
  switchTab(event) {
    ...
    this.setData({
      currentTab: cur,
      navScrollLeft: (cur - 2) * singleNavWidth,
      childid: this.data.navData[cur].WeekCategoryId
    });
  },
  // 跳转详情页
  goDetail(event) {
  ...
  onShareAppMessage: function() {
    ...
    // console.log('市集列表luser',luser.rsShopId)
    if (luser) {
      return {
        title: this.data.datalist.title,
        path: "/pages/home/weekcategory/weekcategory?WeekCategoryId=" + this.data.id + '&isChild=1&scene=' + luser.rsShopId + '&title=' + this.data.title + '&childid=' + this.data.childid+'&currentTab='+this.data.currentTab,
        imageUrl: 'http://cache.jiangxinxiaozhen.com//WD/APP/shareimg.png?v=1'
      }
    } else
      return false;
  }
