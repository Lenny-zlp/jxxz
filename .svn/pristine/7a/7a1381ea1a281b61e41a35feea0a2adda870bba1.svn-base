...
Page({
 ...
 var user = wx.getStorageSync('user')
  ...
  _callback: function(res) {
    // console.log("res",res)
    const styleid = res.product.styleId
    const HotProductid = res.product.HotProductId
    const {HotProductSort,HotName,HotProductSortString,ProductServiceNames} = res.product
    let serveList = ProductServiceNames.split(",")
    this.setData({HotProductid,HotProductSort,HotProductSortString,HotName,serveList,styleid})
    wx.hideLoading({fail:(err)=> {console.log(err)}})
    var timeNum = ""
  ...
  moreSale: function() {
    ...
    request({url:'/Product/ProductCUXIAO_V2',data})
    .then(res=> {
      if(res.data.state.returnCode == "1") {
        this.setData({
          saleList: res.data.data.list,
        })
      }else {
        wx.showToast({
  ...
  onServer: function() {
    ...
    var server = _product.getProductBute('/Product/ProductCUXIAO_V2?sku=' + id.sku + '&styleid='+id.styleid, this._dCallback);
  },
